namespace :sync do
  ...
  desc "syncronize all harvest data"
  task :harvest, [:subdomain] => :environment do |t, args|
    subdomain = args[:subdomain] || SubmarineAccount.first.subdomain
    syncronize(sync: "+all", account_id: SubmarineAccount.where(:subdomain => subdomain).first.id)
  end

  desc "syncronize harvest time"
  task :time, [:subdomain, :days_ago, :days] => :environment do |t, args|
    ...
    syncronize(date_from: days_ago.days.ago, date_to: (days_ago - days).days.ago, sync: "+time", account_id: SubmarineAccount.where(:subdomain => subdomain).first.id)
  end

  desc "syncronize all external services"
  ...
  def syncronize(data = {})
    ...
    begin
      ...
      log << "ProjectAssignments"
      activity.message = "Syncronizing ... \n" + log.join("\n")
      activity.save
      Project.where(:update_associations => true).first(25).each do |project|
        log += HarvestTaskAssignment.syncronize_project_task_assignments(sync_mode, project)
        activity.message = "Syncronizing ... \n" + log.join("\n")
        activity.save

        log += HarvestUserAssignment.syncronize_project_user_assignments(sync_mode, project)
        activity.message = "Syncronizing ... \n" + log.join("\n")
        activity.save

        project.update_associations = false
        project.save
      end if ["all", "+all", "tasks", "+time"].include?(data[:sync])
